    ...
    <>
      <aside className={`${openSidebar == true ? "w-64 absolute inset-0 z-1" : "w-0"} overflow-hidden md:w-64 shrink-0 bg-gradient-to-b from-blue-400 to-indigo-200 text-white  flex md:flex flex-col transition-all duration-300 dark:from-blue-700 darak:to-indigo-500`} >
        ...
        <nav className="flex-1 px-4 space-y-2">
          ...
          {/* <Link to="/admin/addBanner" onClick={() => { setOpenSidebar(false) }} className={`flex items-center gap-3 p-3 rounded-lg cursor-pointer transition-all hover:bg-white/20 hover:bg-white/20 hover:translate-x-1 `}>
            <FaMoneyCheckAlt /> Add Banner
          </Link> */}
          {/* <Link to="/admin/addItem" onClick={() => { setOpenSidebar(false) }} className={`flex items-center gap-3 p-3 rounded-lg cursor-pointer transition-all hover:bg-white/20 hover:bg-white/20 hover:translate-x-1 `}>

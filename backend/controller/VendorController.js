...
const addCategoryfunction = async (req, res) => {
    try {
        ...
        const { categoryName, description, imageFile, status,catPriority,imgPublicId,pimage,userName } = req.body;
        ...
        const category = new CategoryModel({ categoryname:categoryName, description, status,email,catpriority:catPriority,imgPublicId,pimage,userID:_id});
        // const image = await Image.create(req.body);
        // user.password = await bcrypt.hash(password, 10);
        await category.save();
